...
#include <omp.h> // Dodajanje OpenMP
...
int main() {
  ...
  #pragma omp parallel for shared(A, b, T) // Paralelizacija zanke
  for (int i = 0; i < n; i++) {
    double a = 0.0;
    for (int j = 0; j < n; j++) {
      if (j != i) {
    ...
    T[i] = (b[i] - a) / A[i][i]; // Uporaba diagonalnih elementov za iterativni izračun
  }
